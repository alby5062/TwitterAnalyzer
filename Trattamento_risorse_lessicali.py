...
def risorse_lessicali():
    ...
    with open('C:/Users/alby5/PycharmProjects/TwitterAnalyzer/Resource/risorse_lessicali.json', 'w') as fp:
        json.dump(emo_dict, fp, indent=4)
        print("---- Json file: 'risorse_lessicali.json' created ----")

    # -------- CREATE LIST OF FILE NAME WITH VALUE 0 AS DEFAULT ------------
    # freq_emo_dict = dict()
    filenames = dict()
    for subdir,dirs,files in os.walk(root):
        if subdir != root:
    ...
    # -------- CREATE THE DICTIONARY OF WORDS WITH ASSOCIATED VALUES ------------
